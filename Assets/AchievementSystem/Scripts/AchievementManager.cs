...
public class AchievementManager : MonoBehaviour
{
    ...
    public void Unlock(int Index)
    {
        if (!States[Index].Achieved)
        {
            ...
            //Reward Player Skin
            PlayerPrefs.SetInt(AchievementList[Index].Key.ToString(), 1);
            Debug.Log($"{AchievementList[Index].Key}, {PlayerPrefs.GetInt(AchievementList[Index].Key)}");
            SkinUnlock(Index);
            DisplayUnlock(Index);
            AutoSaveStates();
    ...
    private void SkinUnlock(int index)
    {
